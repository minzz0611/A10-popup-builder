    ...
    <div style={{display:'grid',gridTemplateColumns:`repeat(${cols},1fr)`,gap:14}}>
      {(d.items||[]).map((it,i)=>(
        <div key={i} style={{background:'#fff',border:'1px solid var(--line)',borderRadius:14,padding:'16px 18px',boxShadow:'var(--shadow-sm)'}}>
          <div style={{display:'inline-block',background: it.gradient ? `linear-gradient(90deg, ${it.gradientFrom||'#7B5CFA'} 0%, ${it.gradientTo||'#2FA8FF'} 100%)` : (it.badgeColor||'#2F6BFF'),color:'#fff',fontWeight:800,fontSize:13,padding:'6px 14px',borderRadius:999,marginBottom:10}}>
  ...
  if(t === 'badge-card'){
    return (
      <div>
        ...
        <Field label="카드">
          <ArrayEditor items={d.items} onChange={(v)=>set('items',v)} itemLabel="카드"
            defaultItem={{badge:'배지 텍스트를 입력하세요',badgeColor:'#2F6BFF',gradient:false,gradientFrom:'#5601FF',gradientTo:'#2882FF',desc:'설명을 입력하세요.'}}
            renderItem={(it,upd)=>(
              <div style={{display:'grid',gap:6}}>
                ...
                {it.gradient ? (
                  <>
                    <div style={{fontSize:10.5,color:'var(--mute)',fontWeight:700}}>시작 색상</div>
                    <ColorPicker value={it.gradientFrom} onChange={(v)=>upd({gradientFrom:v})}/>
                    <div style={{fontSize:10.5,color:'var(--mute)',fontWeight:700}}>끝 색상</div>
                    <ColorPicker value={it.gradientTo} onChange={(v)=>upd({gradientTo:v})}/>
                  </>
                ) : (
